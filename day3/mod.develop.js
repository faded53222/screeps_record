...
var modDevelop = {
    run: function(spawn) {
        ...
        if(spawn.memory.lv==1 && spawn.room.controller.level>=2){
            ...
            build_near([spawn.pos.x,spawn.pos.y],5,STRUCTURE_EXTENSION,2);
        }
                ...
                if(spawn.pos.findClosestByRange(w)!=null){
                    ...
                    if(k_y!=-1){
                        ...
                        for(var i=0;i<50;i++){
                            ...
                            if(things.length<=2 && (things[0]['type']=='terrain' || things[1]['type']=='terrain') && things[0]['terrain']!='wall' && things[1]['terrain']!='wall'){
        ...
        if(spawn.memory.capacity_lv==3 && spawn.room.energyCapacityAvailable>=1800){
           ...
           for(var name in Game.creeps){
                if(Game.creeps[name].memory.lv<5 && (Game.creeps[name].memory.role==1 || Game.creeps[name].memory.role==2) ){
                    Game.creeps[name].Recycle();
                }
            }
        }
        if(spawn.memory.lv==5){
            spawn.memory.lv+=1;
            for(var c=0;c<spawn.memory.harvest_pos.length;c++){
               spawn.addUrgentTask([0,5,[spawn.memory.sources_list[c][0],spawn.memory.sources_list[c][1]],
               [spawn.memory.harvest_pos[c][0],spawn.memory.harvest_pos[c][1]],[spawn.room.memory.links_list[c][0],spawn.room.memory.links_list[c][1]]]);  
            }
        } 
            spawn.work();
    }
